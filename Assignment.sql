...
#5Who is the best customer? The customer who has spent the most money will be declared the best customer.  OR
...
SELECT * FROM popular_genre WHERE RowNo <= 1;
 ...
 #Determine which customer has spent the most on music for each country. OR
...
JOIN invoice i ON i.invoice_id IN (
    SELECT invoice_id 
    FROM invoice_line
    WHERE track_id IN (
        SELECT track_id 
...
WHERE alb.artist_id = bsa.artist_id
